...
export default function FinancialAnalysis({ simulationSummaryRef }: { simulationSummaryRef: React.RefObject<SimulationSummary> }) {
  ...
  useEffect(() => {
    if (!session?.idToken || !simulationSummaryRef.current || hasFetched.current) return;
    hasFetched.current = true; // Ensure fetch runs only once
    ...
    fetchAnalysis();
  }, [simulationSummaryRef.current]); // Ensure proper dependency tracking
      ...
      <div className="container mx-auto max-w-6xl px-6">
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {data.Summary?.map((item, index) => (
            <div key={index} className="flex items-center p-6 rounded-2xl border border-gray-700 shadow-lg">
              {item.includes("✅") ? (
              ...
              <span className="text-lg font-medium">{item.slice(2)}</span>
            </div>
          ))}
        </ul>
        ...
        <div className="grid grid-cols-1 mb-16 md:grid-cols-2 gap-6">
          {data.Trend?.map((item, index) => {
            ...
            const isDebtOrExpense = item.includes("Debt") || item.includes("Expense");

            return (
              <div key={index} className="flex items-center p-6 rounded-2xl border border-gray-700 shadow-lg">
                {isDebtOrExpense ? (
                  isPositive ? <TrendingUp className="mr-4 text-red-400 w-8 h-8" /> : <TrendingDown className="mr-4 text-green-400 w-8 h-8" />
                ) : isPositive ? (
